...
export const deleteUser = onCall(async (request) => {
  ...
  const applications = await firestore.collection(APPLICATIONS).where('userId', '==', uid).get()
  applications.docs.forEach((doc) => batch.delete(doc.ref))

  // Delete memos authored by user
  const memos = await firestore.collection(MEMOS).where('authorId', '==', uid).get()
...
export const approveStakeWardChange = onCall(async (request) => {
  ...
  if (callerRole !== 'admin') {
    ...
    if (
      callerRole === 'bishop' &&
      (callerStake !== requestData.requestedStake || callerWard !== requestData.requestedWard)
    ) {
...
async function cascadeDeleteConference(conferenceId: string): Promise<void> {
  ...
  for (let i = 0; i < applicationIds.length; i += 30) {
    ...
    const memoSnap = await firestore.collection(MEMOS).where('applicationId', 'in', chunk).get()
...
export const cleanupDeactivatedConferences = onSchedule('every day 00:00', async () => {
  ...
  const snap = await firestore.collection(CONFERENCES).where('isActive', '==', false).get()
