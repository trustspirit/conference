...
export default function AdminRoles() {
  ...
  const handleRoleChange = (uid: string, newRole: string | string[]) => {
    ...
    openConfirm(t('common.confirm'), `${t('common.confirm')}: ${roleLabel}?`, () => {
      updateRole.mutate(
        { uid, role },
        {
          onSuccess: () =>
            toast({ variant: 'success', message: `${roleLabel} ${t('common.approved', '완료')}` }),
  ...
  const handleLeaderStatusToggle = (uid: string, current: LeaderStatus | null) => {
    ...
    updateLeaderStatus.mutate(
      { uid, leaderStatus: next },
      {
        onSuccess: () =>
          toast({
            variant: 'success',
            message: next === 'approved' ? t('admin.roles.approved') : t('admin.roles.pending')
          }),
        onError: () => toast({ variant: 'danger', message: t('errors.generic') })
      }
    )
  ...
  const handleDeleteUser = (uid: string, name: string) => {
    openConfirm(t('common.delete'), t('admin.confirmDelete', { name }), () => {
      deleteUser.mutate(uid, {
        onSuccess: () =>
          toast({
            variant: 'success',
            message: t('accountSettings.deleteUsers.deletedSingle', { userName: name })
          }),
        onError: () => toast({ variant: 'danger', message: t('errors.generic') })
      })
      closeConfirm()
  ...
  return (
    <div className="mx-auto max-w-7xl p-6">
      <h1 className="text-2xl font-bold text-gray-900 mb-1">
        {t('admin.roles.title', '역할 관리')}
      </h1>
      <p className="text-sm text-gray-500 mb-6">
        {t('admin.roles.subtitle', '사용자 역할 및 권한을 관리합니다.')}
      </p>
          ...
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                {t('admin.roles.columns.name', 'Name')}
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                {t('admin.roles.columns.email', 'Email')}
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                {t('admin.roles.columns.role', 'Role')}
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                {t('admin.roles.columns.leaderApproval', 'Leader Approval')}
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
              ...
              <tr key={user.uid}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  {isSelf(user.uid) ? (
                    <div className="flex items-center gap-2">
                      <Badge
                        variant={TONE_TO_BADGE[getRoleTone(user.role)] || 'secondary'}
                        size="sm"
                      >
                        {t(ROLE_LABELS[user.role!] || 'roles.applicant')}
                      </Badge>
                      <span className="text-xs text-gray-400">
                        {t('admin.roles.cannotChangeRole', '자신의 역할은 변경 불가')}
                      </span>
                    </div>
                  ) : (
                    <Select
                    ...
                    <Switch
                      ...
                      label={
                        user.leaderStatus === 'approved'
                          ? t('admin.roles.approved', '승인됨')
                          : t('admin.roles.pending', '대기 중')
                      }
                      size="sm"
                    />
                  ) : (
