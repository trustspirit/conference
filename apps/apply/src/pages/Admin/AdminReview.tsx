...
export default function AdminReview() {
  ...
  const goToPrev = () => {
    if (hasPrev) setSelectedId(filteredItems[currentIndex - 1].key)
  }
  const goToNext = () => {
    if (hasNext) setSelectedId(filteredItems[currentIndex + 1].key)
  }
  ...
  const navigateToNext = () => {
    ...
    if (nextItem) {
      ...
      toast({
        variant: 'info',
        message: t('admin.review.allReviewed', '모든 검토를 완료했습니다.')
      })
    }
  }
  ...
  const executeStatusChange = () => {
    ...
    const onSuccess = () => {
      toast({
        variant: 'success',
        message: t('admin.review.updateStatus', { name: item.name }) + ` → ${statusLabel}`
      })
      navigateToNext()
    }
    ...
    if (item.type === 'application') {
      updateAppStatus.mutate(
        { id: item.entityId, status: newStatus as ApplicationStatus },
        { onSuccess, onError }
      )
    } else {
      updateRecStatus.mutate(
        { id: item.entityId, status: newStatus as RecommendationStatus },
        { onSuccess, onError }
      )
    }
  ...
  return (
    <div className="mx-auto max-w-7xl p-6">
      <div className="flex items-center justify-between mb-2">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {t('admin.review.title', '신청서 검토')}
          </h1>
          <p className="text-sm text-gray-500">
            {t('admin.review.subtitle', '들어오는 신청서를 관리하고 상태를 업데이트하세요.')}
          </p>
        </div>
      ...
      <Tabs value={activeTab} onChange={setActiveTab}>
        <Tabs.List>
          <Tabs.Trigger value={ADMIN_REVIEW_TABS.ALL}>
            {t('admin.review.tabs.all', '전체')} ({tabCounts.all})
          </Tabs.Trigger>
          <Tabs.Trigger value={ADMIN_REVIEW_TABS.AWAITING}>
            {t('admin.review.tabs.awaiting', '대기 중')} ({tabCounts.awaiting})
          </Tabs.Trigger>
          <Tabs.Trigger value={ADMIN_REVIEW_TABS.APPROVED}>
            {t('admin.review.tabs.approved', '승인됨')} ({tabCounts.approved})
          </Tabs.Trigger>
          <Tabs.Trigger value={ADMIN_REVIEW_TABS.REJECTED}>
            {t('admin.review.tabs.rejected', '다음 기회에')} ({tabCounts.rejected})
          </Tabs.Trigger>
        </Tabs.List>
      </Tabs>
                ...
                <div style={{ display: 'flex', gap: '0.25rem' }}>
                  ...
                  <Badge variant={item.type === 'application' ? 'info' : 'success'} size="sm">
                    {item.type === 'application'
                      ? t('admin.review.tags.applied', '신청')
                      : t('admin.review.tags.recommended', '추천')}
                  </Badge>
                  <Badge
                    variant={TONE_TO_BADGE[STATUS_TONES[item.rawStatus] || 'draft'] || 'secondary'}
                    size="sm"
                  >
          ...
          <>
            ...
            <Drawer.Content>
              <p style={{ fontSize: '0.75rem', color: '#9ca3af', marginBottom: '1rem' }}>
                {t('admin.review.submitted', '제출됨')}:{' '}
                {selected.createdAt instanceof Date ? selected.createdAt.toLocaleDateString() : ''}
              </p>
              ...
              <div style={{ marginTop: '1.25rem' }}>
                ...
                <p style={{ fontSize: '0.875rem', color: '#111827' }}>
                  {selected.moreInfo ||
                    t('admin.review.noAdditionalInfo', '추가 정보가 제공되지 않았습니다.')}
                </p>
              </div>

              {/* Current Status */}
              <div
                style={{ marginTop: '1.5rem', paddingTop: '1rem', borderTop: '1px solid #e5e7eb' }}
              >
                <p
                  style={{
                    fontSize: '0.75rem',
                ...
                <Badge
                  variant={
                    TONE_TO_BADGE[STATUS_TONES[selected.rawStatus] || 'draft'] || 'secondary'
                  }
                >
      ...
      <Dialog
        ...
        onClose={() => setConfirmDialog({ open: false, item: null, newStatus: '' })}
        size="sm"
      >
        <Dialog.Title onClose={() => setConfirmDialog({ open: false, item: null, newStatus: '' })}>
          {t('common.confirm')}
        </Dialog.Title>
        <Dialog.Content>
          <p style={{ fontSize: '0.875rem', color: '#374151' }}>
            {confirmDialog.item &&
              `${t('admin.review.updateStatus', { name: confirmDialog.item.name })} → ${t(`admin.review.status.${confirmDialog.newStatus}`, confirmDialog.newStatus)}`}
          </p>
        </Dialog.Content>
        <Dialog.Actions>
          <Button
            variant="outline"
            onClick={() => setConfirmDialog({ open: false, item: null, newStatus: '' })}
          >
            {t('common.cancel', '취소')}
          </Button>
          <Button variant="primary" onClick={executeStatusChange}>
            {t('common.confirm', '확인')}
          </Button>
        </Dialog.Actions>
      </Dialog>
