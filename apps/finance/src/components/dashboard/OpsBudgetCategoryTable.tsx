...
import {
  ...
  useDeleteCategoryWithInclusions
} from '../../hooks/queries/useOpsBudget'
import { UNIQUE_BUDGET_CODES } from '../../constants/budgetCodes'
import type { OpsBudgetCategory, OpsBudgetInclusion, Project } from '../../types'
...
export default function OpsBudgetCategoryTable({ project, inclusions, currentUser }: Props) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setNewName('')
      setNewAlloc(0)
      setNewCode(UNIQUE_BUDGET_CODES[0])
    } catch {
      /* persist already toasted; leave form open */
    }
  }
  ...
  const handleDelete = async (cat: OpsBudgetCategory) => {
    ...
    const next = categories.filter((c) => c.id !== cat.id).map((c, i) => ({ ...c, sortIndex: i }))
    try {
    ...
    <>
      {redistributeState && (
        <OpsBudgetRedistributeModal
          ...
          totalKrw={
            redistributeState.mode === 'reduce-total'
              ? redistributeState.newTotal
              : redistributeState.totalKrw
          }
      ...
      <div className="finance-panel rounded-lg p-4 sm:p-6 mt-6">
          ...
          <div className="flex-1">
            ...
            {editingTotal ? (
              <div className="flex items-center gap-2 mt-1">
                ...
                />
                <button onClick={handleSaveTotal} className="text-xs text-finance-primary">
                  {t('common.save')}
                </button>
                <button
                  onClick={() => {
                    setEditingTotal(false)
                    setTempTotal(totalKrw)
                  }}
              ...
              <div className="flex items-center gap-3 mt-1">
                ...
                {totalKrw > 0 && (
                  <span
                    className={`text-xs ${sumAllocated > totalKrw ? 'text-finance-danger' : 'text-finance-muted'}`}
                  >
                    (
        ...
        <div className="sm:hidden space-y-3">
          {categories.map((cat) => {
            ...
            return (
              <div key={cat.id} className="rounded-lg border border-finance-border p-4">
                {editing ? (
                  <div className="space-y-3">
                    <label className="block">
                      <span className="text-xs text-finance-muted">
                        {t('dashboard.opsBudget.colName')}
                      </span>
                      <input
                        value={draft!.name}
                        onChange={(e) => setDraft({ ...draft!, name: e.target.value })}
                        className="mt-1 w-full border rounded px-2 py-2 text-base"
                      />
                    </label>
                    <label className="block">
                      <span className="text-xs text-finance-muted">
                        {t('dashboard.opsBudget.colBudgetCode')}
                      </span>
                      <select
                        value={draft!.budgetCode}
                        onChange={(e) =>
                          setDraft({ ...draft!, budgetCode: Number(e.target.value) })
                        }
                        className="mt-1 w-full border rounded px-2 py-2 text-base"
                      >
                    ...
                    <label className="block">
                      <span className="text-xs text-finance-muted">
                        {t('dashboard.opsBudget.colAllocated')}
                      </span>
                      <input
                        type="number"
                        value={draft!.allocatedKrw || ''}
                        onChange={(e) =>
                          setDraft({ ...draft!, allocatedKrw: Number(e.target.value) || 0 })
                        }
                        className="mt-1 w-full border rounded px-2 py-2 text-base text-right"
                      />
                    </label>
                    <div className="flex gap-2">
                      <button
                        className="flex-1 rounded border border-finance-border py-2 text-sm text-finance-muted"
                        onClick={() => {
                          setEditingId(null)
                          setDraft(null)
                        }}
                      ...
                      <button
                        className="finance-primary-button flex-1 rounded py-2 text-sm"
                  ...
                  <>
                    <div className="flex items-center justify-between gap-2">
                      <span className="min-w-0 flex items-center gap-2 font-medium text-sm">
                        <span
                        ...
                        <span className="shrink-0 font-mono text-xs text-finance-muted">
                          {cat.budgetCode}
                        </span>
                      </span>
                      <span className="flex shrink-0 gap-1">
                        <button
                          className="rounded px-2.5 py-1.5 text-xs text-finance-primary hover:bg-finance-primary-subtle"
                          onClick={() => {
                            setEditingId(cat.id)
                            setDraft({ ...cat })
                          }}
                        ...
                        <button
                          className="rounded px-2.5 py-1.5 text-xs text-finance-danger hover:bg-red-50"
                    ...
                    <dl className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                      <div className="flex justify-between gap-2">
                        <dt className="text-finance-muted">
                          {t('dashboard.opsBudget.colAllocated')}
                        </dt>
                        <dd>{`₩${cat.allocatedKrw.toLocaleString('en-US')}`}</dd>
                      </div>
                      <div className="flex justify-between gap-2">
                        <dt className="text-finance-muted">
                          {t('dashboard.opsBudget.colIncluded')}
                        </dt>
                        <dd>{`₩${t1.includedKrw.toLocaleString('en-US')}`}</dd>
                      </div>
                      <div className="flex justify-between gap-2">
                        <dt className="text-finance-muted">USD</dt>
                        <dd>
                      ...
                      <div className="flex justify-between gap-2">
                        <dt className="text-finance-muted">
                          {t('dashboard.opsBudget.colRemaining')}
                        </dt>
                        <dd
                      ...
                      <div className="flex justify-between gap-2">
                        <dt className="text-finance-muted">{t('dashboard.opsBudget.colUsage')}</dt>
                        <dd>{(t1.usageRatio * 100).toFixed(0)}%</dd>
                      </div>
                    </dl>
                  </>
                )}
          ...
          {adding && (
            <div className="rounded-lg border border-finance-border p-4 space-y-3">
              <label className="block">
                <span className="text-xs text-finance-muted">
                  {t('dashboard.opsBudget.colName')}
                </span>
          ...
          {categories.length > 0 && (
            <dl className="rounded-lg bg-finance-surface border border-finance-border p-4 grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
              <div className="col-span-2 font-medium mb-1">{t('dashboard.opsBudget.totals')}</div>
              <div className="flex justify-between gap-2">
                <dt className="text-finance-muted">{t('dashboard.opsBudget.colAllocated')}</dt>
                <dd className="font-semibold">{`₩${totals.grandAllocatedKrw.toLocaleString('en-US')}`}</dd>
              </div>
              <div className="flex justify-between gap-2">
                <dt className="text-finance-muted">{t('dashboard.opsBudget.colIncluded')}</dt>
                <dd className="font-semibold">{`₩${totals.grandTotalKrw.toLocaleString('en-US')}`}</dd>
              </div>
              <div className="flex justify-between gap-2">
                <dt className="text-finance-muted">USD</dt>
              ...
              <div className="flex justify-between gap-2">
                <dt className="text-finance-muted">{t('dashboard.opsBudget.colRemaining')}</dt>
                <dd
          ...
          <FinanceTable variant="embedded" minWidthClassName="min-w-[720px]">
            <FinanceTable.Head>
              <tr>
                ...
                <FinanceTable.Th size="compact" align="right">
                  {t('dashboard.opsBudget.colUsage')}
                </FinanceTable.Th>
                <FinanceTable.Th size="compact" align="right">
                  {t('common.actions')}
                </FinanceTable.Th>
            ...
            <FinanceTable.Body>
              {categories.map((cat) => {
                ...
                const editing = editingId === cat.id && draft
                return (
                  <FinanceTable.Row key={cat.id} hover>
                    ...
                    >
                      {editing ? (
                        <span className="flex gap-1 justify-end">
                          ...
                          <button className="text-xs text-finance-primary" onClick={handleSaveEdit}>
                            {t('common.save')}
                          </button>
                        </span>
                      ) : (
                        <span className="flex gap-2 justify-end">
                          ...
                          <button
                            className="text-xs text-finance-danger"
                            onClick={() => handleDelete(cat)}
                          >
                            {t('common.delete')}
                          </button>
              ...
              {categories.length === 0 && !adding && (
                <FinanceTable.Row hover={false}>
                  <FinanceTable.Td
                    ...
                    colSpan={8}
                    align="center"
                    className="text-finance-muted py-6"
                  >
                    {t('dashboard.opsBudget.noCategories')}
                  </FinanceTable.Td>
                </FinanceTable.Row>
                ...
                <tr>
                  ...
                  <FinanceTable.Td size="compact" align="right" className="font-semibold">
                    {totals.grandTotalUsd
                      ? `$${totals.grandTotalUsd.toLocaleString('en-US')}`
                      : '-'}
                  </FinanceTable.Td>
                  <FinanceTable.Td
                    size="compact"
          ...
          <FinanceTable variant="embedded">
            <FinanceTable.Head>
              <tr>
                ...
                <FinanceTable.Th size="compact" align="right">
                  {t('dashboard.opsBudget.opsAllocated')}
                </FinanceTable.Th>
              </tr>
            </FinanceTable.Head>
            <FinanceTable.Body>
