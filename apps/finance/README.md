...
항목별 예산 코드, 교통비 자동 계산, 영수증 첨부, 실시간 검증 체크리스트를 지원합니다.

![신청서 작성](docs/screenshots/request-form.png)

### 내 신청 내역

상태별(대기/검토완료/승인/정산/반려) 필터링과 무한 스크롤을 지원합니다.

![내 신청 내역](docs/screenshots/my-requests.png)

### 신청 관리 (관리자)

전체 신청서를 조회하고, 상태별 필터링 및 정렬이 가능합니다. 검토/승인 비고가 함께 표시됩니다.

![신청 관리](docs/screenshots/admin-requests.png)
...
사용자 목록 조회, 권한 변경(9단계 역할), 사용자 삭제 기능을 제공합니다.
