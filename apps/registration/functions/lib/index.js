...
exports.submitRegistration =
  exports.findCodeByEmail =
  exports.lookupByCode =
  exports.sendPersonalCode =
    void 0
...
async function checkRateLimit(ip, config) {
  const docRef = db.collection(RATE_LIMIT_COLLECTION).doc(`${config.prefix}_${ip}`)
  ...
  if (doc.exists) {
    ...
    await docRef.update({
      ...
      dailyResetAt: isNewDay ? admin.firestore.Timestamp.fromDate(todayStart) : data.dailyResetAt
    })
  } else {
    await docRef.set({
      lastSentAt: admin.firestore.Timestamp.now(),
...
// ─── sendPersonalCode ─────────────────────────────────────────────
exports.sendPersonalCode = (0, https_1.onCall)(
  ...
  async (request) => {
    ...
    const rawTitle = surveySnap.exists ? surveySnap.data()?.title || 'Registration' : 'Registration'
...
exports.findCodeByEmail = (0, https_1.onCall)(
  { secrets: [gmailEmail, gmailAppPassword] },
  async (request) => {
...
const KNOWN_PARTICIPANT_FIELDS = ['name', 'email', 'phoneNumber', 'gender', 'age', 'stake', 'ward']
exports.submitRegistration = (0, https_1.onCall)(async (request) => {
  ...
  for (const [key, value] of Object.entries(participantData)) {
    if (!KNOWN_PARTICIPANT_FIELDS.includes(key) && value) {
      metadata[key] = value
    }
  }
