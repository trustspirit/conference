...
async function checkRateLimit(ip: string, config: RateLimitConfig): Promise<void> {
...
export const submitRegistration = onCall(async (request) => {
  ...
  if (!surveyId || !data || !participantData) {
    throw new HttpsError('invalid-argument', 'surveyId, data, and participantData are required')
  }
